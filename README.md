...
- 输入问题和背景，由大模型为明确或隐含的选择、开放式建议和未来结果推测尽量列全合理且有实质区别的候选：至少 2 项，不设个数上限，不默认只给两三个；明确二选一时保留两项，全部交给 Jev 评估（仍受请求体大小、超时及模型服务容量限制，结果推测不代表事实或现实发生概率）
...
决策问题会先展示候选，让你选择自己的倾向；如果暂时选不出来，可以选择列表末尾的「犹豫」。Jev 同时在后台独立评估，点击「确认选择，查看 Jev 的建议」后才会揭晓结果；如果仍在处理，页面会继续等待。你的选择不会发送给 Jev，「犹豫」也不参与模型候选和概率分布。结果会同时展示你的选择和 Jev 的建议，历史与复制文本也会保留你的倾向。关闭本轮阅读器会取消等待；普通问答仍直接展示回答。
...
本地启动后访问 [随机实验室](http://localhost:9001/random-preview.html)，可对比 CSS 立体轻动画与 Three.js / cannon-es 物理 3D 动画。此独立页面使用固定示例，不请求 AI、不改动 Jev 问答流程，也不写入答案历史。

选择 2、3、4、5、6 或 8 个候选后，点击「播放对比」，两版会展示同一次等概率抽签的结果。3D 先模拟碰撞，再收束到目标面；不是按物理落点决定概率。页面支持深浅主题、手机布局及系统减少动态效果设置。

已选用物理 3D 版；骰子在最后下落前完成朝向调整，落地后正对用户的面即为结果，结果卡须通过真实网格朝向校验才显示。演示可以重复播放；后续正式随机模式将于 AI 生成候选后自动投掷一次，不提供重投。正式问答流程尚未接入。

3D 源码位于 `preview-src/physics.js`，打包产物随 `public/` 提供，不依赖 CDN。修改该文件或它引用的共享模块后，执行 `npm run build:preview` 重新生成浏览器文件；CSS 轻动画与页面本身直接使用原生 ES 模块。Three.js 与 cannon-es 的许可见 `public/random-preview-licenses.txt`。
