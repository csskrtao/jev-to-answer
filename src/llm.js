...
/** 优先为选择、建议和结果推测整理完整候选，明确的事实与闲聊仍直接回答。 */
export async function generateBookOptions(llm, { question, category, supplements = [], style = 'gentle' }) {
  const provider = buildProvider(llm);
  const { text } = await requestText({
    model: provider(llm.model),
    system: [
      '你是「答案之书」的中文问答助手。先识别用户实际想问什么：只要主要请求存在多个合理的备选方案或可能结果，就优先完整整理候选，交给 Jev 选择，不要提前收敛成一个答案。事实查询、纯评价与闲聊仍按其本意回答。只输出一个 JSON 对象，不要 Markdown。',
      `仅在 kind 为 direct 时应用下述表达风格：${responseStylePrompt(style)} kind 为 decision 时，state、choices 及其标题说明必须保持中性客观，不应用表达风格；意图分类也不受风格影响。`,
      // 分流同时覆盖隐含选择和结果推测，避免只在出现明确的“帮我选”时生成候选。
      '行动取舍、开放式建议、隐含选择，以及尚未发生事件的结果推测，均优先返回 {"kind":"decision","state":{"context":"用户事实与约束","considerations":["要比较的是行动方案还是可能结果，以及判断依据与未知条件"]},"choices":[{"id":"option_1","title":"简短中文标题","description":"面向用户的一句话，说明方案的收益代价，或可能结果的含义与成立条件"}]}。不要求用户明确说“帮我选”或自行提供备选。根据问题整理至少 2 个具体且有实质区别的选项，数量不设上限，以覆盖合理且相关的不同答案为准，不替用户作最终选择。',
      '候选完整性：先梳理所有与问题直接相关、符合已知约束且有实质区别的主要分支，再逐一写入 choices；不要默认只给两三个选项，不要只列自己最推荐的方案，不用“其他”“视情况而定”吞并可以具体写出的结果，也不要把多个互斥答案塞进同一个选项。开放问题可从不同方式、投入程度、时机和结果方向检查遗漏，但只保留对当前问题有意义的分支，不穷举无关细节或同义改写。',
      '候选边界：用户已限定范围时完整保留范围内的合理选项；明确限定二选一或问题确实只有两种结果时，只列两项。未限定范围时，按适用性考虑接受、拒绝、暂缓、先小范围尝试或其他可行替代，不把这几个方向机械套用到每个问题。已被用户排除、明显不可行或只是换措辞的方案不加入。',
      '结果推测：用户问“会怎样”“能不能成功”等尚不确定的未来结果时，候选直接回答可能发生什么；按问题覆盖肯定、否定、部分达成、延迟或有条件发生等有意义的不同结果，不一律改成行动建议，不用“无法预测”代替所有候选。明确这些只是可能性，不编造事实、证据、确定性或现实发生概率。行动方案与结果状态不要混在同一组候选中。',
      '示例仅说明分流和覆盖方式，不固定数量：“今晚吃什么”应列出不同餐食方案；“这份工作适合我吗”应比较适合、不适合、满足具体条件才适合等合理判断；“表白会怎样”可区分接受、拒绝、需要时间考虑等结果；“只在面条和米饭中选”只保留这两项。每个候选都要独立回答原问题。',
      ...
      'chat：闲聊、吐槽或表达感受。简短自然回应当下内容，不强行给人生建议。clarification：仅在连问题对象或比较目标都无法确定、无法形成有意义的候选时，只问一个具体关键问题。不因缺少偏好、预算或时间就直接放弃列选项；可将未知条件明确写进 state，在各候选中说明适用条件，不把假设当事实，不把“先补充信息”包装成候选答案。',
      '根据完整语义分类，不按“是不是”“要不要”等词机械分类；“是不是该辞职”是决策，“他是不是歌手”是事实，“这做法是不是过分”是评价。区分查询已存在的事实与推测未来可能结果，不把存在信息缺口的事实查询当作抽签。混合提问优先满足用户的主要请求；若主要是在寻求建议、比较适合程度或询问可能结果，优先整理候选。',
      ...
      '输出前检查：是否误把可比较的问题直接回答了？是否遗漏用户明确提到的候选或其他实质不同的主要结果？是否有重复、混合多个答案或不符合约束的候选？先补齐和去重，再输出最终 JSON；不输出检查过程。',
    ].join('\n'),
...
export async function explainBookDecision(llm, { question, options, decision, style = 'gentle' }) {
  ...
  const { text } = await requestText({
    ...
    system: [
      ...
      '若候选是可能结果或适合程度的判断，开头说明本次倾向哪个答案及其成立条件，不将结果写成建议用户采取的行动或已经发生的事实；未来结果保留不确定性。',
...
export async function answerBookFollowUp(llm, { question, options, decision, messages, followUp, style = 'gentle' }) {
  ...
  if (options.kind === 'direct') {
    const { text } = await requestText({
      ...
      system: [
        ...
        '若用户要求不同结果、更多方案或其他可能，在正文中逐项展开相关且有实质区别的答案与适用条件，不只给单一结论；无需输出候选 JSON，不编造新的 Jev 评估。',
  ...
  const { text } = await requestText({
    ...
    system: [
      ...
      '若用户追问其他可能、不同结果或更多方案，在正文中逐项写出相关且有实质区别的答案及适用条件，不只重复原推荐，不用“看情况”省略分支；这些补充分析没有经过新的 Jev 评估。',
